...
export default async function CategoryLessonsPage(props: { params: Promise<{ categorySlug: string }> }) {
    ...
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: '#FFF8DC' }}>
      ...
      <main className="relative flex-grow pt-32 pb-16">
        <div className="max-w-4xl mx-auto px-4">
          <Link href="/Education" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors mb-8">
          ...
          <div className="mt-16 flex flex-col items-center gap-6">
            {category.lessons?.map((lesson) => (
              <Link
                key={lesson._id}
                href={`/Education/${categorySlug}/${lesson.slug.current}`}
